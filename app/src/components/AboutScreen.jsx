...
export default function AboutScreen({ open = true, onClose }) {
  ...
  useEffect(() => { if (!open) setDiagnostics(false); }, [open]);
  ...
  return (
    <div className={`screen about-screen${closing ? ' is-closing' : ''}`}>
      ...
      <section className="about-section">
        ...
        <button className="diagnostics-toggle" onClick={() => setDiagnostics((v) => !v)} aria-expanded={diagnostics}>Troubleshooting details</button>
        ...
        {diagnostics && <p className="version">
          {platform} · Native player: {Capacitor.isPluginAvailable('TrailerPlayer') ? 'active' : 'MISSING'}
      ...
      <section className="about-section">
        <h3>Your data</h3>
        <p>
          Trailer Roulette has no accounts or analytics SDKs. Preferences stay on your
          device. Movie and playback requests go to third-party services, including
          TMDB and YouTube. YouTube may show its own ads.
        </p>
        <p>
          <PolicyLinks />
        </p>
      ...
      <section className="about-section">
        <h3>Attribution</h3>
        <a href="https://www.themoviedb.org/" target="_blank" rel="noreferrer"><img src="/tmdb-logo.svg" alt="TMDB" width="100" height="40" /></a>
        <p>
      ...
      <section className="about-section">
        <h3>Contact</h3>
        <p><a href={LINKS.support} target="_blank" rel="noreferrer">Support and help</a></p>
